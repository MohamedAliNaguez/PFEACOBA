...
test.describe('Distrobutor Installers menu tests', () => {
 ...
 test('Add new installer', async ({ page }) => {
        ...
        // Generate phone number not starting with 1, 6, or 0
        const firstDigit = ['2', '3', '4', '5', '7', '8', '9'][Math.floor(Math.random() * 7)];
        const phone = firstDigit + Math.floor(1000000 + Math.random() * 9000000).toString();
